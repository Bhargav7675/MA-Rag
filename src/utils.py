...
def load_ir_dataset(name, split):
    """Load queries and corpus via ir_datasets (legacy helper)."""
...
def load_benchmark_dataset(name):
    from src.env import benchmark_dataset_path

    return load_jsonl(str(benchmark_dataset_path(name)))
...
def load_corpus(name="dpr"):
    if name == "dpr":
        ...
        from src.env import get_pubmed_corpus_path

        pubmed_path = get_pubmed_corpus_path()
        if pubmed_path is None:
            raise FileNotFoundError(
                "PubMed corpus not found. Set MA_RAG_PUBMED_CORPUS or place pubmed.tsv.gz under data/."
            )
        DL_DOCS = GzipExtract(LocalDownload(str(pubmed_path)))
