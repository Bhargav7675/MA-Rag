...
class Retriever:
    ...
    def init_index_and_add(self, root_dir, dataset_name):
        ...
        for i, part in enumerate(partitions):
            ...
            # break
        use_gpu = os.getenv("MA_RAG_USE_FAISS_GPU", "auto").lower()
        if use_gpu == "auto":
            use_gpu = "1" if faiss.get_num_gpus() > 0 else "0"
        if use_gpu in ("1", "true", "yes"):
            self._move_index_to_gpu()
        else:
            logger.info("Using FAISS CPU index (MA_RAG_USE_FAISS_GPU=0 or no GPU)")
